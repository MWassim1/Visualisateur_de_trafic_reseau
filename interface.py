...
def  init(srciP,dstiP,protocol,destMac,srcMac,srcPort,dstPort,times,comment):
    ...
    x=100
    p = 70
    dico_ip = {} # Va nous permettre d'associer une adresse iP avec une position "x"
    b=0
    p_x = 50 
    p_y = 80
    t = nb_ip*180
...
def filtre(event):
    ...
    if(protocol_selected.get()=="All Flows" and ip_selected.get()=="All iP") : 
        ...
        if(protocol_selected.get()=="All Flows") : # tout les flux associé à une adresse iP source sélectionnée 
        ...
        elif(ip_selected.get()=="All iP"): # toutes les adresses iP sources associées à une flux sélectionné
            for i in range(0,len(list_srcI)):
                if(protocol_selected.get()==list_protocol[i]):
                    ...
                    l_times.append(list_times[i])
                    l_comment.append(list_comment[i])
        else : # une adresse iP source associée à un flux en particulier 
            for i in range(0,len(list_srcI)): 
